...
CCallableDotAPlayerAdd *CGHostDBMySQL :: ThreadedDotAPlayerAdd( uint32_t gameid, uint32_t colour, uint32_t kills, uint32_t deaths, uint32_t buildkills, uint32_t win, uint32_t newcolour)
{
	void *Connection = GetIdleConnection( );
	...
	CCallableDotAPlayerAdd *Callable = new CMySQLCallableDotAPlayerAdd( gameid, colour, kills, deaths, buildkills, win, newcolour, Connection, m_BotID, m_Server, m_Database, m_User, m_Password, m_Port );
	CreateThread( Callable );
	m_OutstandingCallables++;
	return Callable;
}
...
uint32_t MySQLDotAPlayerAdd( void *conn, string *error, uint32_t botid, uint32_t gameid, uint32_t colour, uint32_t kills, uint32_t deaths, uint32_t buildkills, uint32_t win, uint32_t newcolour)
{
	uint32_t RowID = 0;
	string Query = "INSERT INTO dotaplayers ( botid, gameid, colour, kills, deaths, buildkills, win, newcolour) VALUES ( " + UTIL_ToString( botid ) + ", " + UTIL_ToString( gameid ) + ", " + UTIL_ToString( colour ) + ", " + UTIL_ToString( kills ) + ", " + UTIL_ToString( deaths ) + ", " + UTIL_ToString( buildkills ) + ", " + UTIL_ToString( win ) + ", "+ UTIL_ToString( newcolour ) + " )";
...
//	string Query = "select totgames,wins,losses,killstotal,deathstotal,creepkillstotal,creepdeniestotal,assiststotal,neutralkillstotal,towerkillstotal,raxkillstotal,courierkillstotal,kills,deaths,creepkills,creepdenies,assists,neutralkills,towerkills,raxkills,courierkills, ("+formula+") as totalscore,server from(select *, (kills/deaths) as killdeathratio, (totgames-wins) as losses from (select gp.name as name,ga.server as server,gp.gameid as gameid, gp.colour as colour, avg(dp.courierkills) as courierkills, sum(dp.raxkills) as raxkillstotal, sum(dp.towerkills) as towerkillstotal, sum(dp.assists) as assiststotal,sum(dp.courierkills) as courierkillstotal, sum(dp.creepdenies) as creepdeniestotal, sum(dp.creepkills) as creepkillstotal,sum(dp.neutralkills) as neutralkillstotal, sum(dp.deaths) as deathstotal, sum(dp.kills) as killstotal,avg(dp.raxkills) as raxkills,avg(dp.towerkills) as towerkills, avg(dp.assists) as assists, avg(dp.creepdenies) as creepdenies, avg(dp.creepkills) as creepkills,avg(dp.neutralkills) as neutralkills, avg(dp.deaths) as deaths, avg(dp.kills) as kills,count(*) as totgames, SUM(case when((dg.winner = 1 and dp.newcolour < 6) or (dg.winner = 2 and dp.newcolour > 6)) then 1 else 0 end) as wins from gameplayers as gp, dotagames as dg, games as ga,dotaplayers as dp where dg.winner <> 0 and dp.gameid = gp.gameid and dg.gameid = dp.gameid and dp.gameid = ga.id and gp.gameid = dg.gameid and gp.colour = dp.colour and gp.name='"+name+"' group by gp.name) as h) as i";
	...
	string Query2 = "SELECT place from ladder WHERE login='"+name+"'";
	uint32_t TotalBuildKills = 0;
if( mysql_real_query( (MYSQL *)conn, Query2.c_str( ), Query2.size( ) ) == 0 )
{
 MYSQL_RES *Result = mysql_store_result( (MYSQL *)conn );
 if( Result )
 {
  vector<string> Row = MySQLFetchRow( Result );
  if( Row.size( ) == 1 )
   TotalBuildKills = UTIL_ToUInt32( Row[0] );
  mysql_free_result( Result );
 }
}

	string Query = "SELECT COUNT(a.id) as totGames, SUM(kills) as kills, SUM(deaths) as deaths,SUM(buildkills) as buildkills, SUM(win) as win , SUM(case when(win = 0) then 1 else 0 end) as loses FROM dotaplayers AS a LEFT JOIN gameplayers AS b ON b.gameid = a.gameid and a.colour = b.colour where name='"+name+"'";
		...
		if( Result )
		{
			...
			if( Row.size( ) == 6 )
			{
				...
				{					
					...
					//uint32_t TotalBuildKills = UTIL_ToUInt32( Row[3] );
					uint32_t TotalWins = UTIL_ToUInt32( Row[4] );
					uint32_t TotalLosses = UTIL_ToUInt32( Row[5] );
                    double WinRate = ((double)TotalWins/(double)TotalGames)*100;
					//uint32_t TotalBuildKills = ((TotalWins*(TotalKills/5))+(TotalGames*100)/(TotalLosses+TotalDeaths)*10) ;
					//CONSOLE_Print(  "wins = "+UTIL_ToString(TotalWins)+" games= "+UTIL_ToString(TotalGames)+ " winrate= "+UTIL_ToString((TotalWins/TotalGames)*100));
					//CONSOLE_Print(  UTIL_ToString(WinRate));
					/*
					...
					*/
					DotAPlayerSummary = new CDBDotAPlayerSummary( string( ), name, TotalGames, TotalKills, TotalDeaths, TotalBuildKills,TotalWins,TotalLosses,WinRate);
				}
			}
			else;
...
void CMySQLCallableDotAPlayerAdd :: operator( )( )
{
	...
	if( m_Error.empty( ) )
		m_Result = MySQLDotAPlayerAdd( m_Connection, &m_Error, m_SQLBotID, m_GameID, m_Colour, m_Kills, m_Deaths, m_BuildKills, m_Win,m_NewColour);

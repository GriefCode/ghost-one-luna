		...
		if( i->second->GetReady( ) )
		{
			...
			if (sd)
			{
				...
				//	RankS = RankS + "/" + UTIL_ToString(scorescount);
					...
					Summary = m_GHost->m_Language->HasPlayedDotAGamesWithThisBot(i->second->GetName( ),
																						...
																						UTIL_ToString( DotAPlayerSummary->GetTotalBuildKills( ) ),
																						UTIL_ToString( DotAPlayerSummary->GetTotalWins( ) ),
																						UTIL_ToString( DotAPlayerSummary->GetTotalLosses( ) ),
																						UTIL_ToString( DotAPlayerSummary->GetWinRate( ),2 )  );
					if (!Whisper)
						SendAllChat(Summary);
					else
					{
					...
					if (PlayerN) PlayerN->m_CachedSTATS = Summary;
				}
			}
			if (!sd)
			if( DotAPlayerSummary )
	...
	if( AdminCheck || RootAdminCheck )
	{
		...
		if( !m_Locked || RootAdminCheck || IsOwner( User ) )
		{
			...
			******************/
			...
			if( ( Command == "abort" || Command == "a" || Command == "ô" ) && m_CountDownStarted && !m_GameLoading && !m_GameLoaded && !m_NormalCountdown )
			{
...
/*
	...
	*********************/
	...
	if( Command == "stf" && GetTime( ) >= player->GetStatsDotASentTime( ) + 5 && !m_GHost->m_nostatsdota )
	{
		string StatsUser = User;

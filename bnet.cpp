	...
	for( vector<PairedDPSCheck> :: iterator i = m_PairedDPSChecks.begin( ); i != m_PairedDPSChecks.end( ); )
	{
		if( i->second->GetReady( ) )
		{
			...
			if( DotAPlayerSummary )
			{
				...
				QueueChatCommand( m_GHost->m_Language->HasPlayedDotAGamesWithThisBot(  i->second->GetName( ),
...
//				m_GHost->m_DB->BanRemove( Forgiven, 1 );
	...
	if( !m_CallableAdminList && GetTime( ) >= m_LastAdminRefreshTime + 2900 )
	{
...
//			if( IsAdmin( User ) || IsRootAdmin( User ) || m_OutPackets.size( ) <= 3 )
			if( m_OutPackets.size( ) <= 3 )
			{
				...
				if( Command == "stf" && (GetTime()-m_LastStats>=5) && !m_GHost->m_nostatsdota )
				{
					m_LastStats = GetTime();
					string StatsUser = User;
...
{
	gameName = gameName + UTIL_ToString(m_BNLSWardenCookie & 7);
	if( hostName.empty( ) )
	{
